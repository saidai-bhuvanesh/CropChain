...
describe("CropChain", function () {
  ...
  it("Should set the right owner and grant DEFAULT_ADMIN_ROLE", async function () {
    ...
    expect(await cropChain.hasRole(DEFAULT_ADMIN_ROLE, owner.address)).to.be
      .true;
  });
  ...
  it("Should allow admin to grant stakeholder roles", async function () {
    ...
    expect(await cropChain.hasRole(TRANSPORTER_ROLE, transporter.address)).to.be
      .true;
  ...
  it("Should prevent non-admin from granting roles", async function () {
    await expect(
      cropChain
        .connect(farmer)
        .grantStakeholderRole(FARMER_ROLE, other.address),
    ).to.be.revertedWith(
  ...
  it("Should allow proper supply chain progression with role-based stage transitions", async function () {
    ...
    await expect(
      cropChain.connect(mandi).updateBatch(
        batchId,
        1, // Mandi Stage
        "Mandi Market",
        "Iowa Market",
    ...
    await cropChain
      .connect(mandi)
      .approveCustodian(batchId, transporter.address);
    await expect(
      cropChain.connect(transporter).updateBatch(
        batchId,
        2, // Transport Stage
        "TransCo",
        "In Transit",
    ...
    await cropChain
      .connect(transporter)
      .approveCustodian(batchId, retailer.address);
    await expect(
      cropChain.connect(retailer).updateBatch(
        batchId,
        3, // Retailer Stage
        "Supermarket",
        "Chicago",
  ...
  it("Should prevent invalid stage jumps and unauthorized role transitions", async function () {
    ...
    await expect(
      cropChain.connect(farmer).updateBatch(
        batchId,
        1, // Mandi Stage
  ...
  it("Should allow admin to bypass role restrictions for updates", async function () {
    ...
    await expect(
      cropChain.connect(owner).updateBatch(
        batchId,
        1, // Mandi Stage
        "Admin Update",
        "Admin Location",
  ...
  describe("Input Validation", function () {
    ...
    it("Should revert when notes exceed maximum length", async function () {
      ...
      await expect(
        cropChain
          .connect(farmer)
          .createBatch(
            toBytes32String("BATCH-VAL-004-EMPTY"),
            cropTypeHash,
            validIpfsCID,
            quantity,
            "Farmer Joe",
      ...
      await expect(
        cropChain
          .connect(farmer)
          .createBatch(
            toBytes32String("BATCH-VAL-004-MAX"),
            cropTypeHash,
            validIpfsCID,
            quantity,
            "Farmer Joe",
    ...
    it("Should enforce validation on batch updates", async function () {
      ...
      await expect(
        cropChain
          .connect(mandi)
          .updateBatch(batchId, 1, "", "Iowa Market", "Received goods"),
      ).to.be.revertedWith("Actor name length invalid");
      ...
      await expect(
        cropChain
          .connect(mandi)
          .updateBatch(batchId, 1, "Mandi Market", "", "Received goods"),
      ).to.be.revertedWith("Location length invalid");
      ...
      await expect(
        cropChain
          .connect(mandi)
          .updateBatch(batchId, 1, "Mandi Market", "Iowa Market", longNotes),
      ).to.be.revertedWith("Notes too long");
  ...
  describe("Oracle Integration Tests", function () {
    ...
    beforeEach(async function () {
      ...
      await cropChain.grantStakeholderRole(FARMER_ROLE, farmer.address);
      await cropChain.grantStakeholderRole(MANDI_ROLE, mandi.address);
      await cropChain.grantStakeholderRole(
        TRANSPORTER_ROLE,
        transporter.address,
      );
      await cropChain.grantStakeholderRole(ORACLE_ROLE, oracle.address);
    ...
    it("Should allow Transporter to request IoT verification", async function () {
      const tx = await cropChain
        .connect(transporter)
        .requestIoTVerification(testBatchId);
    ...
    it("Should allow Oracle to fulfill IoT data with optimal conditions", async function () {
      ...
      const tx = await cropChain
        .connect(oracle)
        .fulfillIoTData(testBatchId, temperature, humidity);
    ...
    it("Should prevent IoT fulfillment for non-existent batch", async function () {
      ...
      await expect(
        cropChain.connect(oracle).fulfillIoTData(nonExistentBatchId, 650, 45),
